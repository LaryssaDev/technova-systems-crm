...
const Finance: React.FC<{ store: any }> = ({ store }) => {
  ...
  const [partners, setPartners] = useState<Partner[]>([
    { name: 'Laryssa Ferreira', percentage: 30 },
    { name: 'Lucas Barbosa', percentage: 15 },
    { name: 'TechNova Systems - Reserva', percentage: 55 },
  ]);
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-[36px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              <div>
                ...
                <div className="mt-3 flex items-center justify-between">
                  <div className="flex-1 h-2 bg-slate-800 rounded-full overflow-hidden mr-4">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${Math.abs(totalPercentage - 100) < 0.01 ? 'bg-emerald-500' :
                          totalPercentage > 100 ? 'bg-rose-500' : 'bg-blue-500'
                        }`}
                      style={{ width: `${Math.min(100, totalPercentage)}%` }}
                    />
                  </div>
                  <span className={`text-sm font-black tabular-nums ${Math.abs(totalPercentage - 100) < 0.01 ? 'text-emerald-400' :
            ...
            <div className="p-6 border-t border-slate-800 flex gap-3">
              ...
              <button
                ...
                className={`flex-[2] py-3.5 rounded-2xl font-black text-sm uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-3 shadow-xl ${generatingPDF
